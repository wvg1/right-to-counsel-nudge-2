...
def relabel_files(cause, case_folder):
    ...
    try:
        ...
        file_dict = {old: file_dict[old] for old in file_dict if '.pdf' in old.lower()}
...
def download_case_documents(driver, case_number, base_url, output_dir):
    ...
    try:
        ...
        # wait for a new zip to appear and no .crdownload files active
        sleep_time = 0
        while sleep_time < 300:
            crdownloads = glob.glob(os.path.join(output_dir, '*.crdownload'))
            current_zips = set(glob.glob(os.path.join(output_dir, '*.zip')))
            new_zips = current_zips - existing_zips
...
def main():
    ...
    PROCESS_ZIPS_EVERY = 100
    RESTART_CHROME_EVERY = 200
